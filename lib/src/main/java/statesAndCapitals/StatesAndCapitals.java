...
import static org.checkerframework.checker.nullness.Opt.map;
...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        StateInfo cardinalState = states.stream().filter(s->s.getStateBird().equals("cardinal")).findFirst().get();
        //getStateBird=="cardinal";
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream().map(s->s.getCapital()).collect(averagingDouble(s->s.getAverageYearlyPrecipitationInInches()));
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().map(s->s.getCapital()).collect(summingInt(s->s.getAverageYearlyPrecipitationInInches()));
        ...
        Map<String, Long> numberOfStatesByTimeZone =  states.stream().map(s->s.getTimeZones()).collect(groupingBy());
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().collect(groupingBy(s -> s.getCapital().getTimeZone(), Collectors.counting()));
        ...
        List<String> stateTreesSortedAscending = states.stream().map(s->s.getStateTree()).sorted().collect(toList());
        ...
        String allStateNamesSemicolonDelimited = states.stream().map(s->s.getStateName()).collect(Collectors.joining(";"));
        ...
        List<String> allDistinctStateBirds = states.stream().map(s->s.getStateBird()).distinct().collect(Collectors.toList());
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds =  states.stream().map(s->s.getStateBird()).distinct().filter(b->!b.equals("mockingbird")).collect(Collectors.toList());
        ...
        Long numberOfDistinctStateBirds = states.stream().map(s->s.getStateBird()).distinct().collect(Collectors.counting());
